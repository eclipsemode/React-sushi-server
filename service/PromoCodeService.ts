...
class PromoCodeService {
    async create({code, discount, type, limit}: Promocode) {
    ...
    async findOne({code}: {code: string}) {
    ...
    async getAll({page, size, match}: IGetAll) {
    ...
    async change({code, newValue}: {code: string, newValue: string}) {
        const foundCode = await Promocode.findOne({where: {code: code.toLowerCase()}});
